...
describe MediTAF::Utils::Email::Mail do
  ...
  context "single email with url" do
    ...
    it 'should find an invite email' do
      expect(@invite).to be_a MediTAF::Utils::Email::Message
    end
    ...
    it 'should raise an error when the invite email is not found' do
      @mail.delete $config['utils']['gmail']['gmail_user_addr'], 'You have been invited to join iMedidata'
      expect { @mail.find_email($config['utils']['gmail']['gmail_user_addr']).email }.to raise_error MediTAF::Utils::Email::InviteEmailNotFoundError
    end

    it 'should raise an error when the subject is nil on delete' do
      expect { @mail.delete $config['utils']['gmail']['gmail_user_addr'] }.to raise_error MediTAF::Utils::Email::SubjectNil
    end
  end
  ...
  context "multiple emails" do
    before (:each) do
      ...
      @mail = MediTAF::Utils::Email::Mail.new(@email_addr, $config['utils']['gmail']['gmail_user_password'])
    end
    ...
    it 'should raise an error when the URL is not found' do
      ...
      expect { @noinvite.url }.to raise_error MediTAF::Utils::Email::URLNotFoundError
    end
  ...
  context "A specific email recipient in an email thread" do
    ...
    it 'should find an email sent to a particular email address from multiple emails in a thread' do
      ...
      expect(@findParticularUser).to be_a MediTAF::Utils::Email::Message
    ...
    it 'should find an email sent to a particular email address' do
      ...
      expect(@inviteParticularUser).to be_a MediTAF::Utils::Email::Message
    end
  end
end
